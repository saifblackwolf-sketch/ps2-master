...
namespace DarwinMisc {
    extern int Alune_CRASH_DIAG;
    extern int Alune_REC_DIAG;
    extern int Alune_FORCE_EE_INTERP;
    extern int Alune_FORCE_JIT_VERIFY;
    extern int Alune_CALL_TGT_X9;
    extern int Alune_CRASH_PACK;
    extern int Alune_WX_TRACE;
    extern int Alune_CALLPROBE;
    extern int Alune_JIT_HLE;        // [P11] JIT modeの HLE enabled/disabled (default=1=enabled, 0=disabled)
    extern int Alune_FORCE_JIT;      // [P11] JIT modeforce (1=force, 0=configに従う; SAFE_ONLY not needed)
    extern int Alune_IOP_CORE_TYPE;  // [P11] IOP CPU select (-1=EEfollow, 0=psxRecforce, 1=psxIntforce)
    
    // [Alune] Indirect Branch Probe
    ...
    // [Alune] W^X State Tracker (0=RX, 1=RW)
    extern volatile int g_jit_write_state;
    // [Alune] Recompiler Stage Tracker
...
struct CPUClass {
    ...
    // [Alune] DYLD Main Base Getter
    void LogDyldMain();

    // [Alune] JIT Block Mapping Service
