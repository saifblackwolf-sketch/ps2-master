...
// [Alune] P0 Cleanup: Disable behavior-changing patches by default.
// This gates BiosTools (CopyBIOS patches) and Memory (memReset ROM patches).
#ifndef Alune_ENABLE_P0_BEHAVIOR_PATCHES
#define Alune_ENABLE_P0_BEHAVIOR_PATCHES 0
#endif
...
// Use this for Alune_* runtime flags so launch-time flags are reflected reliably.
inline const char* Alune_GetRuntimeEnv(const char* name)
{
	if (!name || !name[0])
		return nullptr;
	...
	if (std::strncmp(name, "Alune_", 7) != 0)
		return nullptr;
...
inline bool Alune_GetRuntimeEnvBool(const char* name, bool default_value = false)
{
	const char* value = Alune_GetRuntimeEnv(name);
	if (!value)
...
inline bool Alune_IsSafeOnlyEnabled()
{
	static int s_cached = -1;
	if (s_cached < 0)
		s_cached = Alune_GetRuntimeEnvBool("Alune_SAFE_ONLY", true) ? 1 : 0;
	return (s_cached == 1);
}
